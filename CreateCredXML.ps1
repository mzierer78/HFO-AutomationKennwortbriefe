...
If ($Logfile -eq "Default"){
    $global:Logfile = Join-Path $here -ChildPath "EPMMonitoringCreateCreds.log"
}
...
$CredXML = Join-Path $here -ChildPath "credentials.xml"
...
$Credential = $host.ui.PromptForCredential("Need credentials", "Please specify credentials to authenticate with EPM", "", "")
